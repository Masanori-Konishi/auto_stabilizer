...
public:
  ...
  double overwritableRemainTime = 0.20; // 0以上. 単位[s]. 次indexまでの残り時間がこの値を下回っている場合、着地位置時間修正を行わない. DOWN_PHASEのときにfootstepNodesList[0]のdstCoordsはgenCoordsよりも高い位置に変更されることがないようにするために、LegCoordsGeneratorのdelayTimeOffset以上の値にせよ.
  ...
  std::vector<std::vector<cnoid::Vector3> > overwritableStrideLimitationHull = std::vector<std::vector<cnoid::Vector3> >{std::vector<cnoid::Vector3>{cnoid::Vector3(0.45,-0.15,0),cnoid::Vector3(-0.45,-0.15,0),cnoid::Vector3(-0.45,-0.350,0),cnoid::Vector3(-0.20,-0.45,0),cnoid::Vector3(0.20,-0.45,0),cnoid::Vector3(0.45,-0.350,0)},std::vector<cnoid::Vector3>{cnoid::Vector3(0.45,0.350,0),cnoid::Vector3(0.20,0.45,0),cnoid::Vector3(-0.20,0.45,0),cnoid::Vector3(-0.45,0.350,0),cnoid::Vector3(-0.45,0.15,0),cnoid::Vector3(0.45,0.15,0)}}; // 要素数2. 0: rleg用, 1: lleg用. 着地位置修正時に自動生成されるfootstepの上下限の凸包. 反対の脚のEndEffector frame(Z軸は鉛直)で表現した着地可能領域(自己干渉やIKの考慮が含まれる). あったほうが扱いやすいのでZ成分があるが、Z成分は0でないといけない. 凸形状で,上から見て半時計回り. thetaの影響はlegHullとlegCollisionMarginを用いて別で評価されるので、defaultStrideLimitationHullでは考慮しなくて良い. 斜め方向の角を削るなどして、IKが解けるようにせよ. 歩行中は急激に変更されない
